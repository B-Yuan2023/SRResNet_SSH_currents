...
parser.add_argument("--dir_lr", type=str, default="../out2d_8", help="directory of lr dataset")
parser.add_argument("--dir_hr", type=str, default="../out2d_128", help="directory of hr dataset")
parser.add_argument("--up_factor", type=int, default=16, help="upscale factor") # match with dirs
...
suf = '_res' + str(opt.residual_blocks) + '_max_var1_t25'
suf0 = '_res' + str(opt.residual_blocks) + '_max_var1'

rtra = 0.25 # ratio of training dataset

ivar_lr = [3,] # default use only ssh [3,3,3]; [3,4,5] corresponds to ssh, ud, vd
ivar_hr = [3,] 
# varm_lr = find_maxmin_global(opt.dir_lr, ivar)
# varm_hr = find_maxmin_global(opt.dir_hr, ivar)
# estimated based on 70% of whole dataset

varm_hr0 = np.array([[ 3.0, -3.0],
       [ 2.1, -2],
       [ 2.1, -2]])
# varm_hr0 = np.array([[ 2.80240449, -2.63572259],
#        [ 1.93372906, -1.64462934],
#        [ 2.05186204, -1.39972807]])
# varm_lr = np.array([[ 2.71783665, -2.62399791],
#        [ 1.93372906, -1.52233336],
#        [ 1.00687134, -0.96283311]])
