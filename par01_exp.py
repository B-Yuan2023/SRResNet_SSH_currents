...
parser.add_argument("--dir_hr", type=str, default="out2d_16", help="directory of hr dataset")
...
parser.add_argument("--hr_height", type=int, default=16, help="high res. image height")
parser.add_argument("--hr_width", type=int, default=16, help="high res. image width")
...
parser.add_argument("--nlm", type=int, default=2, help="loss function") #
opt = parser.parse_args(sys.argv[2:])
print(opt)
    # return opt

krand = 1   # add randomness or not, 1: yes, different results; 0 same results
nrep = 1   # number of repeated runs

suf = '_res' + str(opt.residual_blocks)   # for output dir name
suf0 = '_res' + str(opt.residual_blocks)  # only used when input existing nn models

rtra = 0.75 # ratio of training dataset

ivar_lr = [3,] # default use only ssh [3,]; [3,4,5] corresponds to ssh, ud, vd
ivar_hr = [3,] 
...
# var range used to convert the data to [0,1]; ssh, u, v, uw, vw, swh, pwp
